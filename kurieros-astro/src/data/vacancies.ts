...
const buildPay = (hourly: number): VacancyOffer['pay'] => {
  ...
  return {
    ...
    // Y.Eda и Купер (оба используют buildPay) предоставляют опцию
    // мгновенных выплат через встроенный кошелёк (Я.Кошелёк / СберPay) —
    // курьер может вывести заработанное в любой день, иногда несколько
    // раз за смену. Формальный пэйрол по-прежнему еженедельный, но для
    // соискателя UX ближе к ежедневному. Помечаем «Ежедневно» — это
    // совпадает с user-intent запроса «работа курьером с ежедневной
    // оплатой» и убирает рассинхрон URL/контента на
...
const _initialSources = [
  ...
  },
  {
    ...
    company: {
      name: BURGER_KING_COMPANY_NAME,
      logo: BURGER_KING_COMPANY_LOGO,
    },
