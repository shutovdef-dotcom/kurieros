...
- geoExpansion: если активны `Москва` и `Санкт-Петербург`, а источник не
  даёт закрытый список и не запрещает область, использовать
  `expandCitiesForCapitalRegions()`; если не используешь — явно записать
  причину в отчёте и тесте/комментарии источника
- applyLink: "#"
- isActive: true
...
Это обязательный pre-publish gate, а не подсказка. Перед сборкой новой
вакансии с `Москва` + `Санкт-Петербург` нужно выбрать и зафиксировать один
из трёх вариантов:

1. `expanded` — применён `expandCitiesForCapitalRegions()`; в тесте или
   локальной проверке подтверждены спутниковые города: минимум
   `Химки`/`Одинцово` для Москвы и `Колпино`/`Пушкин`/`Шушары` для Петербурга.
2. `closed_source_geo` — источник даёт закрытый список конкретных городов,
   и область/спутники добавлять нельзя; ссылку на источник указать в
   комментарии источника или финальном отчёте.
3. `owner_override` — владелец сайта явно сказал не расширять гео; это
   решение нужно процитировать в финальном отчёте.

Если выбран `expanded`, новые локации должны наследовать условия базового
города/лендинга, если источник не даёт специальных ставок. Например, для
удалённой вакансии с лендингом `Европа` у Москвы и Петербурга все
добавленные спутники получают тот же лендинг/UTM-группу.
...
9. **Все города** из offers — либо в `CITY_DATASET`, либо явно подтверждены
   ...
   - **Москва/СПб gate:** если среди активных offers есть `Москва` и
     ...
     находиться `Химки`, `Одинцово`, `Колпино`, `Пушкин`, `Шушары`.
     Для `closed_source_geo` или `owner_override` причина должна быть
     записана в источнике или финальном отчёте.
